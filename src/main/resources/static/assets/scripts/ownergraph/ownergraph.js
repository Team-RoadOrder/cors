...
const SalesChartModule = (function () {
    let salesChart = null;
    let isFetching = false; //데이터 로딩 상태 변수
    ...
    const loadGraph = async (type) => {
        ...
        // 로딩 시작
        isFetching = true;
        if (typeof Loading !== 'undefined') Loading.show("매출 데이터를 분석 중입니다...");
        ...
        try {
            ...
            // 렌더링
            requestAnimationFrame(() => renderChart(data, type));
            ...
            // [중요] 성공/실패 여부와 상관없이 무조건 로딩 종료 및 상태 해제
            isFetching = false;
            if (typeof Loading !== 'undefined') Loading.hide();
        }
    };
    ...
    const init = async () => {
        ...
        // 이미 로드된 데이터가 있거나 로딩 중이면 중복 실행 방지
        if (isFetching || (dataCache.daily && dataCache.monthly)) return;

        isFetching = true;
        // 초기 로드 시에도 사용자에게 진행 상황을 알리고 싶다면 주석 해제
        if (typeof Loading !== 'undefined') Loading.show("데이터 초기화 중...");
        ...
        try {
            ...
            requestAnimationFrame(() => renderChart(dataCache.daily, 'daily'));
